...
if($_GET["keyword"] and $_GET["type"]){
    ...
    for($i=0;$i<count($data);$i++){
    	echo '<div class="headline-background">
        	  <a href="' .$data[$i]['share_url']. '" target="_blank"  class="headline-background-link">
                    <div class="heading">' .substr($data[$i]['ga_prefix'],0,4). '</div>
        	  		<div class="heading-content">' .$data[$i]['title']. '</div>
        	  </a>
        	  </div>';   
    }
}
...
<?php
    if($i == 20){
        echo  '<a target="_self" href="http://zhihudaily.sinaapp.com/search.php?keyword=' .$keyword. '&page=' .++$page. '&type=' .$_GET["type"]. '" class="page-btn">下一页</a>';
    }elseif($page != 1 and $i > 0){
        echo  '<a target="_self" href="http://zhihudaily.sinaapp.com/search.php?keyword=' .$keyword. '&page=' .--$page. '&type=' .$_GET["type"]. '" class="page-btn">上一页</a>';
    }
?>
    </div>
